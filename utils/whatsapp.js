...
const sendOrderNotification = async (order) => {
  try {
    ...
    // ✅ GPS link if coordinates exist
    const mapsLine = order.deliveryAddress?.coordinates?.lat
      ? `\n🗺️ Location: https://www.google.com/maps?q=${order.deliveryAddress.coordinates.lat},${order.deliveryAddress.coordinates.lng}`
      : '';

    // ✅ Coupon line if discount applied
    const couponLine = order.discountAmount > 0
      ? `\n🏷️ Coupon (${order.couponCode}): -₹${order.discountAmount}` : '';
...
📍 *Delivery Address:*
${order.deliveryAddress.fullAddress}
${order.deliveryAddress.landmark ? `Landmark: ${order.deliveryAddress.landmark}` : ''}${mapsLine}
${order.deliveryAddress.pincode ? `Pincode: ${order.deliveryAddress.pincode}` : ''}
...
━━━━━━━━━━━━━━━━━━
💰 Subtotal: ₹${order.subtotal}
🚚 Delivery: ₹${order.deliveryCharge}${couponLine}
💵 *TOTAL: ₹${order.totalAmount}*
━━━━━━━━━━━━━━━━━━
📝 Notes: ${order.notes || 'None'}
${paymentLine}
