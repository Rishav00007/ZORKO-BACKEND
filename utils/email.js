...
const sendOrderEmail = async (order) => {
  try {
    ...
    // ✅ GPS link
    const mapsLine = order.deliveryAddress?.coordinates?.lat
      ? `<br><a href="https://www.google.com/maps?q=${order.deliveryAddress.coordinates.lat},${order.deliveryAddress.coordinates.lng}" target="_blank">🗺️ View delivery location on Google Maps</a>`
      : '';

    // ✅ Coupon line
    const couponLine = order.discountAmount > 0
      ? `<br>🏷️ <b>Coupon (${order.couponCode}):</b> -₹${order.discountAmount}` : '';
    ...
    const html = `
      <div style="font-family: monospace; font-size: 14px; max-width: 500px; margin: auto; background: #f9f9f9; padding: 20px; border-radius: 12px; border: 1px solid #ddd;">
        ...
        <p>📍 <b>Delivery Address:</b><br>
        ${order.deliveryAddress.fullAddress}<br>
        ${order.deliveryAddress.landmark ? `Landmark: ${order.deliveryAddress.landmark}<br>` : ''}${mapsLine}
        ${order.deliveryAddress.pincode ? `Pincode: ${order.deliveryAddress.pincode}` : ''}</p>
        <hr style="border-color:#ddd"/>
        <p>🛒 <b>Items Ordered:</b><br>${itemsList}</p>
        <hr style="border-color:#ddd"/>
        <p>
          💰 Subtotal: ₹${order.subtotal}<br>
          🚚 Delivery: ₹${order.deliveryCharge}${couponLine}<br>
          💵 <b>TOTAL: ₹${order.totalAmount}</b>
        </p>
        <hr style="border-color:#ddd"/>
        <p>📝 <b>Notes:</b> ${order.notes || 'None'}</p>
